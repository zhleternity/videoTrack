...
//  Copyright (c) 2016年 ling. All rights reserved.
//
...
{
    VideoProcessor vp;
    vp.display_input("extract a frame");
    vp.display_output("show out frame");
    double rate = vp.get_frame_rate();
    vp.set_delay(1000/rate);
    vp.stop_frame_num(50);
//    vp.not_callProcess();
    vp.set_input("/Users/eternity/Downloads/IMG_3971.m4v");
    vp.set_frame_processor(canny);
//    vp.set_output("/Users/eternity/Downloads/cyndi",".jpg");
    vp.set_output("/Users/eternity/Downloads/cyndi_out.avi");
    vp.run();
    return 0;
}
#endif
...
int main()
{
    ...
    vp.set_input("/Users/eternity/Documents/avi/test_video/IMG_3971.m4v");
    vp.set_frame_processor(&track);
    vp.display_output("output features");
    vp.set_delay(1000.0/vp.get_frame_rate());
    vp.run();
    return 0;
...
int main()
{
    ...
    for (int i = 10; i < 100; i ++) {
        sprintf(img_name, "/Users/eternity/Documents/study/studyResource /cv/moving tracking/CN/data/extra_sequences/sequences/shirt/img/00%d.png",i);
        imgs.push_back(img_name);
    }

//    VideoCapture video("/Users/eternity/Documents/avi/test_video/cyndi.mp4");
    vp.set_input(imgs);
    vp.set_delay(10);
    if(!vp.is_opened())return 0;
    
    Mat frame,fore_ground;
    namedWindow("extracted foreground");
    bool stop1(false);
    Ptr<BackgroundSubtractorMOG2> mog = createBackgroundSubtractorMOG2(20,16,true);
    //video>>frame;
    while(!stop1){
...
int main()
{
    ...
    corner.push_back(cv::Point(50,10));
    corner.push_back(cv::Point(150,10));
    corner.push_back(cv::Point(50,510));
    corner.push_back(cv::Point(150,510));
    bgfg.set_pr(corner);
    char *img_name = new char[500];
    vector<string> imgs;
    for (int i = 10; i < 100; i ++) {
        sprintf(img_name, "/Users/eternity/Documents/study/studyResource /cv/moving tracking/CN/data/extra_sequences/sequences/surfer/img/00%d.png",i);
        imgs.push_back(img_name);
    }
    //    cout<<imgs[0]<<endl;
    vp.set_input("/Users/eternity/Documents/avi/test_video/IMG_3971.m4v");
    vp.set_frame_processor(&bgfg);
    vp.display_input("Input frame");
    vp.display_output("extracted foreground");
    vp.set_delay(1000./vp.get_frame_rate());
    vp.run();
    return 0;
...
void strLeftMoving(string &str,int n ,int k){
    k = k % n;
    strReverse(str, 0, k-1);
//    cout<<""<<str<<endl;
    strReverse(str, k, n-1);
//    cout<<""<<str<<endl;
...
//int main()
//{
//    VideoCapture cap("/Users/eternity/Documents/avi/test_video/IMG_3971.m4v"); //定义一个摄像头捕捉的类对象
//    Rect trackWindow;
...
//         if( !paused )//没有暂停
//         {
//             cap.read(frame);//从摄像头抓取一帧图像并输出到frame中
//             if( frame.empty())
//                 break;
//         }
//
...
//                 backproj &= mask;
//         
//                //opencv2 .0以后的版本函数命名前没有cv两字了，并且如果函数名是由2个意思的单词片段组成的话，且前面那个片段不够成单词，则第一个字母要大写，比如Camshift，如果第一个字母是个单词，则小写，比如meanShift，但是第二个字母一定要大写
...
//{
//    camTrack cam;
//    cam.cam_tracker("/Users/eternity/Documents/avi/test_video/IMG_3971.m4v");
//    waitKey();
//    return 0;
//}
